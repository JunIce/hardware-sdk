...
export class DH_NVR extends EventEmitter {
  ip: string;
  port: string;
  target: string;
  player: typeof PlayerControl | null;
  ...
  async Login({ username, password }: { username: string; password: string }) {
     ...
     */
    try {
      const res = await RPC.login(username, password, false);
  ...
  getAllCameraList() {
    return RPC.LogicDeviceManager.getCameraAll().then(function (params) {
      ...
      //预览，在线通道列表
      let channelArr = channelList.map((item: any) => {
        let _name;
        item.DeviceInfo.VideoInputs.map((value: any) => {
  ...
  // 获取时间段内视频列表
  async searchPlaybackList({ startTime, endTime, channel }: any) {
    let allRecords: any[] = [];
    let recordNums = 0;
    // let playChannel = $('#h5_playback_channel').value - 0;
    const getMediaFile = (params: any) => {
